...
class DistributedCogThiefGame(DistributedMinigame):
    ...
    def load(self):
        ...
        self.rewardPanel = DirectLabel(
            parent=hidden, relief=None, pos=(
                1.16, 0.0, 0.45), scale=0.65, text='', text_scale=0.2, text_fg=(
                0.95, 0.95, 0, 1), text_pos=(
                0, -.13), text_font=ToontownGlobals.getSignFont(), image=self.jarImage)
        self.rewardPanelTitle = DirectLabel(
            parent=self.rewardPanel, relief=None, pos=(
                0, 0, 0.06), scale=0.08, text=TTLocalizer.CannonGameReward, text_fg=(
    ...
    def initGameWalk(self):
        ...
        if self.useOrthoWalk:
            ...
            def doCollisions(oldPos, newPos, self=self):
                ...
                y = bound(
                    newPos[1], CTGG.StageHalfHeight, -CTGG.StageHalfHeight)
                newPos.setX(x)
                newPos.setY(y)
                return newPos
    ...
    def showToonHitBySuit(self, avId, timestamp):
        ...
        spinHTrack = Sequence(
            LerpHprInterval(
                geomNode, flyDur, destHpr, startHpr=startHpr), Func(
                geomNode.setHpr, startHpr), name=toon.uniqueName('hitBySuit-spinH'))
        ...
        preFunc()
        slipBack = Parallel(Sequence(ActorInterval(toon,
                                                   'slip-backward',
                                                   endFrame=24),
                                     Wait(CTGG.LyingDownDuration -
                                          (flyDur - oldFlyDur)),
                                     ActorInterval(toon,
                                                   'slip-backward',
                                                   startFrame=24)))
    ...
    def updateSuitGoal(self, timestamp, inResponseToClientStamp,
                       suitNum, goalType, goalId, x, y, z):
        if not self.hasLocalToon:
            return
    ...
    def getTossPieInterval(self, toon, x, y, z, h, p, r,
                           power, beginFlyIval=Sequence()):
    ...
    def markBarrelStolen(
            self, timestamp, inResponseToClientStamp, barrelIndex):
        if not self.hasLocalToon:
            return
